...
class TsdmPlot:
    ...
    def draw(
        ...
        for ci, curve in enumerate(ncd1_curves):
            ...
            best_k = max(range(len(curve)), key=lambda k: curve[k])
            ax_bot.plot([best_k], [curve[best_k]], marker="o", color="red", markersize=3)
        ax_bot.set_title("NCD1(Y_k) curves per chunk (red dot = argmax cutoff)")
