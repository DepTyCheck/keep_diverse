...
def overall():
    ...
    kept_files_path = os.path.join(temp_dir, "filtered_files.txt")
    ...
    knee_plot = Plot(
        output_file=filtration_plot_path,
    ...
    filtered_files_list = FilteredFilesList(
        kept_files_path=kept_files_path,
    )
    ...
    stop = (
        ...
        if filter_rounds is None
        else DontStop()
    )

    filter_rounds = filter_rounds if filter_rounds is not None else 420
